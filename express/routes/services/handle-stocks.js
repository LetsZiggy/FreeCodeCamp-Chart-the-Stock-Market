...
/*
  ...
  Object.keys(stocks).forEach((symbol) => {
    ...
    Object.keys(stocks[symbol]['Monthly Adjusted Time Series']).forEach((date) => {
      let year = parseInt(date.split('-')[0]);
      let month = parseInt(date.split('-')[1]);
      // let open = parseFloat(stocks[symbol]['Monthly Adjusted Time Series'][date]['1. open']);
      // let high = parseFloat(stocks[symbol]['Monthly Adjusted Time Series'][date]['2. high']);
      // let low = parseFloat(stocks[symbol]['Monthly Adjusted Time Series'][date]['3. low']);
      let close = parseFloat(stocks[symbol]['Monthly Adjusted Time Series'][date]['4. close']);
      // let volume = parseFloat(stocks[symbol]['Monthly Adjusted Time Series'][date]['5.) volume'];
